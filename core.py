...
class Core:
    ...
    def skip_section(self, section_id: str) -> dict:
        ...
        post_datauser = self.__processor.process_section_data(
            section_data, self.__user.get_username()
        )        
        ...
        while attempts < max_attempts:
            ...
            skipuser_res = self.__request.do_post(
                self.__api_url.skip_sectiontime_user_api(),
                data=json.dumps(post_datauser),
                headers={
            ...
            if skip_res.status_code == 503:
                ...
                try:
                    qes = skip_res.json()
                    print(qes, section_id,qes["code"])
                    status_code = qes["code"]
                except json.JSONDecodeError:
                    print(1,qes)
                    logging.error(f"JSONDecodeError: Empty or invalid JSON response for section_id: {section_id}")
                    return Msg().processing("跳过课程失败，服务器返回无效响应", 400)
    ...
    def skip_sectiontime(self, section_id: str) -> dict:
        section_data = list(
            filter(
                lambda d: d["id"] == section_id,
                self.__user.get_lessons_data()["data"],
        ...
        skip_res = self.__request.do_post(
            self.__api_url.skip_sectiontime_api(),
            data=json.dumps(post_data),
            headers={
                "Referer": "http://www.cqooc.com/learn/mooc/structure?id="
                + section_data["courseId"],
        ...
        statususer_code=skipuser_res.json()["code"]
        status_code = skip_res.json()["code"]
        if status_code == 2 & statususer_code==2:
            return Msg().processing("时长增加失败", 200)
        elif status_code == 0& statususer_code==0:
            return Msg().processing("时长增加成功", 200)
        elif status_code == 1& statususer_code==1:
            return Msg().processing("非法操作", 400)
        else:
            return Msg().processing("时长增加失败", 400)
